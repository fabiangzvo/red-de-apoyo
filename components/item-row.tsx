...
export function ItemRow({
  ...
  function handleReserve() {
    ...
    run(() => reserveItem(item.id, volunteerName))
  }
  ...
  return (
    ...
    >
        ...
        <div className="min-w-0">
          ...
          {item.reservedBy && status !== "pending" && (
            <p className="mt-1 text-xs text-muted-foreground">
              ...
              <span className="font-medium text-foreground">{item.reservedBy}</span>
            </p>
          )}
        </div>
      </div>
      ...
      {status === "pending" && (
        <Button size="sm" className="mt-3 w-full" onClick={handleReserve} disabled={pending}>
          {pending ? (
      ...
      {status === "reserved" && (
        <div className="mt-3 grid grid-cols-2 gap-2">
          <Button size="sm" onClick={() => run(() => deliverItem(item.id))} disabled={pending}>
            {pending ? (
              <LoaderCircle className="size-4 animate-spin" aria-hidden />
            ) : (
